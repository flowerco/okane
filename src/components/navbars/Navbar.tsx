...
export const Navbar = () => {
  ...
  return (
    <nav className="flex flex-col items-center justify-center px-6 sticky top-0 left-0">
      ...
      <div ref={flexContent} className={ `flex items-center h-20 w-full ${expandSearch ? 'justify-center' : 'justify-between'}` } >
        { !expandSearch && (
        ...
        {isAuthenticated && (
          <>
            <form className={ `flex items-center justify-center -ml-3 transition-all ${expandSearch ? 'w-full' : 'w-4/5'}` }>
              <input
                type="text"
                value={searchString}
                ...
                className={ `py-2 pl-6 pr-9 rounded-full bg-[#D9D9D9] text-2xl max-w-md ${expandSearch ? 'w-full' : 'w-4/5'}` }
              ></input>
              <button
                type="submit"
